...
class Masesi extends CI_Model
{

    function getasesi()
    {
        return $this->db->query("SELECT * FROM tb_asesi order by tahun_aktif DESC, kelas ASC, nama ASC")->result();
    }
    ...
    function addasesi($data)
    {
        $this->db->insert('tb_asesi', $data);
    }
    ...
    {
        $query = $this->db->get_where('tb_asesi', ['no_peserta' => $no_pes])->row();
        if (empty($query)) {
            return 'kosong';
        } else {
            return 'ada';
